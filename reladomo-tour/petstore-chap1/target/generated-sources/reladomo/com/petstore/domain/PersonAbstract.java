...
public abstract class PersonAbstract extends com.gs.fw.common.mithra.superclassimpl.MithraTransactionalObjectImpl
{
	...
	public void zPersistDetachedRelationships(MithraDataObject _data)
	{
		PersonData _newData = (PersonData) _data;
		{
			...
			if (pets != null)
			{
				pets.copyDetachedValuesToOriginalOrInsertIfNewOrTerminateIfRemoved();
			}
		}
	}
	...
	*/
	public PetList getPets()
	{
		...
		if (_op != null)
		{
			...
			_result.zSetForRelationship();
			_result.zSetRemoveHandler(TerminateOnRemoveHandler.getInstance());
	...
	public void setPets(PetList pets)
	{
		...
		else if (_behavior.isPersisted())
		{
			...
			petsToDelete.cascadeTerminateAll();
		}
	...
	{
		this.getPets().cascadeTerminateAll();
		this.delete();
	}
	...
	protected int generatePersonId()
	{
		...
		MithraPrimaryKeyGenerator.getInstance().getSimulatedSequencePrimaryKeyGeneratorForNoSourceAttribute("Person", "com.petstore.util.ObjectSequenceObjectFactory",sourceAttribute);
	...
	private void checkAndGeneratePrimaryKeys()
	{
		...
		if (!data.zGetIsPersonIdSet())
		{
			...
			if (pets != null)
			{
				pets.setPersonId(data.getPersonId());
			}
		}
	}
